...
class FusionTable:
    ...
    def Select(self, *columnName):
        if len(columnName) > 0:
            ...
            self.stringBuilder.AppendLine(
                "fusionTable = fusionTable.query(FusionCharts.DataStore.Operators.select([" + columns + "]));")
    ...
    def CreateFilter(self, filterType, columnName, *values):
        ...
        switcher = {
            FusionTable.FilterType.Equals: "FusionCharts.DataStore.Operators.{0}('{1}', '{2}')".format(fx, columnName,
                                                                                                       my_list[0]),
            FusionTable.FilterType.Between: "FusionCharts.DataStore.Operators.{0}('{1}', {2}, {3})".format(fx,
                                                                                                           columnName,
                                                                                                           my_list[0],
                                                                                                           my_list[1])
        }
        ...
        return switcher.get(filterType,
                            "FusionCharts.DataStore.Operators.{0}('{1}', {2})".format(fx, columnName, my_list[0]))
    ...
    def Pipe(self, *filters):
        if len(filters) > 0:
            ...
            self.stringBuilder.AppendLine(
                "fusionTable = fusionTable.query(FusionCharts.DataStore.Operators.pipe(" + filter + "));")
